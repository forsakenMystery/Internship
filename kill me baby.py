...
def weights(shape, mu=0, std=0.1):
...
def model(X_train, Y_train, X_test, learning_rate=0.25):
    x = tf.placeholder(tf.float32, [None, image_size_flat], name="x")
    y = tf.placeholder(tf.float32, [None, number_class], name='y')
    layer1, W1, B1 = new_fc_layer(input=x, num_inputs=X_train.shape[1], num_outputs=int(X_train.shape[1]/2), activation="leaky")
    layer2, W2, B2 = new_fc_layer(input=layer1, num_inputs=int(X_train.shape[1]/2), num_outputs=int(X_train.shape[1]/4), activation="leaky")
    layer3, W3, B3 = new_fc_layer(input=layer2, num_inputs=int(X_train.shape[1]/4), num_outputs=number_class, activation="no")
    layer4, _, _ = new_fc_layer(input=layer3, num_inputs=number_class, num_outputs=number_class, activation="softmax")
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=layer3, labels=y)
    ...
    correct_prediction = tf.equal(layer4, y)
    ...
    def optimize(num_iterations, learning_rate):
        ...
        for i in range(num_iterations):
            ...
            # abb.append(print_accuracy(feed_dict, False))
            if i % 25 is 0 and i is not 0:
                learning_rate /= 2
                if i is 50 or i is 100 or i is 150:
                    learning_rate=0.5
        # plt.plot(np.arange(0, len(abb)), abb, 'r')
        plt.plot(np.arange(0, len(acc)), acc, 'b')
        plt.show()
    optimize(156, learning_rate)
    preds = session.run(tf.argmax(layer4, axis=1), feed_dict={x: X_test})
